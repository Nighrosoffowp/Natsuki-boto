...
var folderUserAuth = "./database/Sessão";
...
sabrina.sendMessage(mdata_2.id, {image: {url:`https://vinizin-apis.online/welcome?titulo=BEM%20VINDO(A)&nome=${sab2.participants[0].split('@')[0]}&perfil=${shortpc.data}&fundo=${fundo1}&grupo=BEM VINDO AO GRUPO ${encodeURI(mdata_2.subject)}`},
...
sabrina.sendMessage(mdata_2.id, {image: {url:`https://vinizin-apis.online/welcome?titulo=Adeus&nome=${sab2.participants[0].split('@')[0]}&perfil=${shortpc.data}&fundo=${fundo2}&grupo=SAIU DE ${encodeURI(mdata_2.subject)}`}, caption: teks, 
...
console.log(colors.red("SESSÃO DO BOT FOI DESCONECTADA, IREI DELETAR A SESSÃO ANTIGA PARA QUE POSSA CONECTAR NOVAMENTE! ^_^"))
exec("cd database && rm -rf Sessão")
...
console.log(colors.green(`〔 - _ NATSUKI - BOT _ MD - 2.5 _ - CONECTADA COM SUCESSO... 〕`))
